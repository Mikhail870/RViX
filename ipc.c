...
// функция примает имя получателя и имя отправителя и оазмер сообщения
// копирует IPC буфер получаетля, отправителю
// парметры берутся из регситров
